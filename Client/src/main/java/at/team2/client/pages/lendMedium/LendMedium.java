...
public class LendMedium extends BasePage<Void, NullType, NullType, NullType>
{
    ...
    private void findBookByID()
    {
        try
        {
            ...
            BookSmallDto foundbook = mainremote.getBookRemoteObject().getBookSmallById(
                    Integer.parseInt(_lendIDfld.getText()));
            if (foundbook != null)
            {
    ...
    private void getCustomerByID()
    {
        try{
            ...
            CustomerSmallDto foundcustomer = mainremote.getCustomerRemoteObject().getCustomerSmallById(
                    customerid);
    ...
    {
        MainRemoteObjectInf mainremote = RmiHelper.getSession();
        LoanRemoteObjectInf loanRemoteObjectInf = mainremote.getLoanRemoteObject();
        LoanSmallDto loanSmallDto = new LoanSmallDto();
        loanSmallDto.set_customerid(_currentCustomer.get_id());
        LocalDate currentlocaldate = LocalDate.now();
        //todo: Get this data from the database instead of hardcoding it
        loanSmallDto.set_start(Date.valueOf(currentlocaldate));
        loanSmallDto.set_end(Date.valueOf(currentlocaldate.plusWeeks(2)));
